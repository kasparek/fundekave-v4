...
if(!empty($_FILES['bann'])) {
	if($ak=upload($_FILES["bann"],WEB_REL_BANNER)) {
		fError::addError(MESSAGE_UPLOAD_SUCCESS);
...
if(isset($_GET['ibd'])) {
    ...
    if(file_exists($banner)) {
    	if($ak = @unlink($banner)){
    	    ...
    	    fError::addError(LABEL_FILE.' '.LABEL_NOTEXISTS.': '.$banner);
    	}
    }
}
//save banner
if(isset($_POST['bid'])) {
	...
	if(empty($arr['linkUrl'])) fError::addError(ERROR_BANNER_TARGETEMPTY);
	$arr['imageUrl'] = Trim($_POST['edhtml']);
	if(empty($arr['imageUrl'])) fError::addError(ERROR_BANNER_EMPTY);
	...
	fHTTP::redirect($user->getUri('ebe='.$bannerId));
}
...
if(!empty($bannerName)) {
  $tpl->setVariable('EDACTION',$user->getUri());
...
if(!empty($arr)) {
    foreach ($arr as $row) {
        ...
        $tpl->setVariable('OWNERNAME',$user->getgidname($row[5]));
        $tpl->setVariable('EDITURL',$user->getUri('ebe='.$row[0]));
        $tpl->setVariable('DELETEURL',$user->getUri('ebd='.$row[0]));
...
while($arrFiles) {
    ...
    if(!in_array($file,$arrUsedFiles)) {
      ...
      $tpl->setVariable('NEWBANNERURL',$user->getUri('ibn='.$file));
      $tpl->setVariable('IMGDELETEURL',$user->getUri('ibd='.$file));
      $tpl->parseCurrentBlock();
    }
}
