...
class page_ItemsList implements iPage
{
    ...
    public static function build($data = array())
    {
        ...
        if (!empty($data['__ajaxResponse'])) {
            ...
            if (isset($grpid)) {
                $cache->setData($data, $dataid, $grpid);
            }

        }
    }

    public static function buildPrep($data = array())
    {
        if (!is_array($data)) {
            $data = array();
        }
        ...
        $manualCurrentPage = 0;
        if (!isset($data['onlyComments'])) {
            $data['onlyComments'] = false;
        }
        ...
        if ($user->idkontrol) {
            if ($pageVO->typeId != 'top') {
                ...
                if ($diff > 0) {
                    ...
                    if ($diff > $pageVO->perPage()) {
                        $pageVO->perPage($diff + 3);
                    }
        ...
        if (!empty($data['__get']['type'])) {
            ...
            if (!isset(FLang::$TYPEID[$typeRequest])) {
                $typeRequest = '';
            }
        ...
        if ($user->categoryVO) {
            $arrPagerExtraVars['c'] = $user->categoryVO->categoryId;
        }

        $arrPagerExtraVars['k'] = $user->pageVO->pageId;
        if (!empty($user->whoIs)) {
            $arrPagerExtraVars['who'] = $user->whoIs;
        }
    ...
    public static function buildList($fItems, $pageVO, $pagerOptions = array(), $overrideTypeId = false)
    {
        $touchedBlocks               = array();
        $vars                        = array();
        ...
        if ($overrideTypeId !== false) {
            $typeId = $overrideTypeId;
        }
            ...
            if ($numItems > 0) {
                ...
                if ($typeId == 'top' || $pageVO->inludeGaleries()) {
                    ...
                    while ($itemVO = array_pop($fItems->data)) {
                        ...
                        if ($index > -1 && ($itemVO->typeId != 'blog' && empty($itemVO->itemIdTop))) {
                            $newArr = array_insert($newArr, $index, $itemVO);
                        } else {
                            array_unshift($newArr, $itemVO);
                        }
                    ...
                    while ($itemVO = array_shift($fItems->data)) {
                        if (!empty($itemVO->itemIdTop)) {
                            ...
                            if ($index > -1) {
                                if (isset($sortedItems[$itemVO->itemIdTop])) {
                                    $sortedItems[$itemVO->itemIdTop]++;
                                } else {
                                    $sortedItems[$itemVO->itemIdTop] = 1;
                                }
                ...
                while ($itemVO = array_shift($fItems->data)) {
                    ...
                    if ($itemVO->typeId == 'event') {
                        if (!in_array('fcalendar', $touchedBlocks)) {
                            $touchedBlocks[] = 'fcalendar';
                        }
                    }
