...
class page_UserSettings implements iPage
{
    ...
    public static function build($data = array())
    {
        ...
        if ($userVO->getXMLVal('settings', 'bookedorder') == 1) {
            $tpl->touchBlock('bookedorder');
        }
        ...
        try {
            ...
            $bs_data = json_decode($json);
        } catch (Exception $e) {
        }
        if (!empty($bs_data->themes)) {
            foreach ($bs_data->themes as $theme) {
