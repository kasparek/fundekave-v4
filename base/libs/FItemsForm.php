...
class FItemsForm
{
    ...
    public static function canComment($pageVO = null, $itemVO = null)
    {
        ...
        if ($pageVO->typeId == 'forum' && $pageVO->locked > 0) {
            return false;
        }
        ...
        if ($pageVO->typeId == 'blog' || $pageVO->typeId == 'galery' || $pageVO->typeId == 'event') {
            ...
            if (!$itemVO && $pageVO->typeId != 'galery') {
                return false;
            }
            ...
            if ($writePerm == 0 || ($writePerm == 2 && !$user->idkontrol)) {
                return false;
            }
    ...
    public static function moveImage($data, $itemVO = null)
    {
        if ($itemVO == null) {
            if (empty($data['i'])) {
                return;
    ...
    public static function process($data)
    {
        ...
        if ($user->itemVO && $user->itemVO->typeId == 'forum') {
            $itemVO->itemIdTop = null;
        }

        if (empty($itemVO->pageIdTop)) {
        ...
        switch ($data['action']) {
            ...
            default:
                ...
                if (!FError::is()) {
                    if (isset($data['addon'])) {
                        $data['addon'] = FText::preProcess($data['addon'], array('plainText' => 1));
                    }
                    //title for blog,event
                    if ($itemVO->typeId != 'galery' && empty($data['addon']) && $itemVO->typeId != 'forum') {
                        FError::add(FLang::$ERROR_NAME_EMPTY);
                    }

                    if (isset($data['name'])) {
                        $data['name'] = FText::preProcess($data['name'], array('plainText' => 1));
                    }

                    if (empty($data['name'])) {
                        $data['name'] = $user->userVO->name;
                    }

                    $data['text'] = FText::preProcess($data['text'], $user->idkontrol ? array() : array('plainText' => 1));
                    if (isset($data['textLong'])) {
                        $data['textLong'] = FText::preProcess($data['textLong']);
                    }

                    if (empty($data['text']) && $itemVO->typeId == 'forum') {
                        FError::add(FLang::$MESSAGE_EMPTY);
                    }

                    if (empty($data['name'])) {
                        FError::add(FLang::$MESSAGE_NAME_EMPTY);
                    } elseif ($user->idkontrol == false) {
                        if (FUser::isUsernameRegistered($data['name'])) {
                            FError::add(FLang::$MESSAGE_NAME_USED);
                        }
                    ...
                    if (isset($data['dateStartLocal'])) {
                        $data['dateStart'] = FSystem::checkDate($data['dateStartLocal'] . (isset($data['dateStartTime']) ? ' ' . $data['dateStartTime'] : ''));
                    }

                    if (isset($data['dateEndLocal'])) {
                        $data['dateEnd'] = FSystem::checkDate($data['dateEndLocal'] . (isset($data['dateEndTime']) ? ' ' . $data['dateEndTime'] : ''));
                    }

                    if (empty($data['dateStart']) && $itemVO->typeId != 'forum' && $itemVO->typeId != 'galery') {
                        FError::add(FLang::$ERROR_DATE_FORMAT);
                    }

                    if (isset($data['location'])) {
                        $data['location'] = FText::preProcess($data['location'], array('plainText' => 1));
                    }
                ...
                if (!FError::is()) {
                    ...
                    if (!empty($data['addon'])) {
                        $itemVO->set('addon', $data['addon']);
                    }

                    if (!empty($data['text'])) {
                        $itemVO->set('text', $data['text']);
                    }

                    if (!empty($data['textLong'])) {
                        $itemVO->set('textLong', $data['textLong']);
                    }

                    if (!empty($data['location'])) {
                        $itemVO->set('location', $data['location']);
                    }

                    if (empty($itemVO->typeId)) {
                        $itemVO->typeId = $user->pageVO->typeId;
                    }

                    if (isset($data['category'])) {
                        if ($itemVO->categoryId > 0) {
                            if ($data['category'] != $itemVO->categoryId) {
                                $oldCategoryId = $itemVO->categoryId;
                            }
                        }
                    ...
                    if (!empty($data['dateStart'])) {
                        $itemVO->set('dateStart', $data['dateStart']);
                    }

                    if (!empty($data['dateEnd'])) {
                        $itemVO->set('dateEnd', $data['dateEnd']);
                    }

                    if (isset($data['public'])) {
                        $itemVO->set('public', (int) $data['public']);
                    }
    ...
    public static function show($itemVO, $data = null)
    {
        $user = FUser::getInstance();
        ...
        if ($itemVO->typeId == 'forum') {
            ...
            if ($opt = FCategory::getOptions($itemVO->pageId, $itemVO->categoryId, true, '')) {
