...
{
    //---close resources
    public static function fin($msg = '', $log = false)
    {
        FSystem::superInvalidateFlush();
        $db = FDBConn::getInstance();
        $db->close();
        if ($log) {
            FError::write_log($log);
        }
    ...
    public static function superInvalidate($grp, $id = '')
    {
        $serialized = $grp . ($id != '' ? '|' . $id : '');
        if (!in_array($serialized, self::$invalidate)) {
            self::$invalidate[] = $serialized;
        }

    }

    public static function superInvalidateFlush()
    {
        if (empty(self::$invalidate)) {
            return;
        }
    ...
    public static function superInvalidateHandle($grps)
    {
        if (empty($grps)) {
            return;
        }
        ...
        foreach ($grps as $grp) {
            $inv = explode('|', $grp);
            if (count($inv) > 1) {
                $cache->invalidateData($inv[1], $inv[0]);
            } else {
    ...
    public static function addSuperVar($key, $val)
    {
        self::$superVars['[[' . $key . ']]'] = $val;
    }

    public static function superVars($data)
    {
        $superVars = array();
        $settings  = FConf::get('settings');
        foreach ($settings as $k => $v) {
            $superVars['[[' . strtoupper($k) . ']]'] = $v;
        }
    ...
    //TEMPLATE HELPER
    public static function isTpl($templatefile, $root = '')
    {
        if ($root == '') {
            $root = LIBS . ROOT_TEMPLATES;
        }

        if (file_exists($root . $templatefile)) {
            return true;
        }

        return false;
    }
    public static function tpl($templatefile, $root = '', $removeUnknownVariables = true, $removeEmptyBlocks = true)
    {
        if ($root == '') {
            $root = LIBS . ROOT_TEMPLATES;
        }
    ...
    public static function tplExist($template)
    {
     ...
     * @return string - URL
     */
    public static function getUri($otherParams = '', $pageId = '', $pageParam = false, $options = array())
    {
        $arrAcnchor  = explode('#', $otherParams);
        $otherParams = $arrAcnchor[0];
        $anchor      = '';
        if (isset($arrAcnchor[1])) {
            $anchor = '#' . $arrAcnchor[1];
        }
        ...
        if (!isset($options['short']) && $newPageId == HOME_PAGE && empty($pageParam)) {
            $newPageId = '';
        }
        ...
        if (!empty($newPageId)) {
            ...
            if (isset($options['name'])) {
                $safeName = FText::safetext($options['name']);
            }
    ...
    public static function processK($pageId)
    {
        if (isset($pageId{5})) {
    ...
    public static function checkDate($date)
    {
        ...
        if ($dateListLen > 3) {
            return null;
        } else if ($dateListLen == 3) {
            $dateTest = $date;
        } else if ($dateListLen == 2) {
            $dateTest = $date . '-01';
        } else if ($dateListLen == 1) {
            $dateTest = $date . '-01-01';
        }

        if (strtotime($dateTest) === false) {
            return null;
        }
    ...
    public static function isTime($time)
    {
        ...
        if (count($arrTime) == 2 || count($arrTime) == 3) {
            ...
            if ($arrTime[1] > 60 && $arrTime[1] < 0) {
                return false;
            }

            if (isset($arrTime[2])) {
                if ($arrTime[2] > 60 && $arrTime[2] < 0) {
                    return false;
                }
    ...
    public static function array_neighbor($key, $arr, $consecutively = false)
    {
        ...
        if (isset($keys[$keyIndexes[$key] - 1])) {
            ...
            if ($consecutively) {
                $return['prev'] = $keys[sizeof($keys) - 1];
            } else {
                $return['prev'] = 0;
            }
            //--- if not previous return last
        }
        //--- next
        if (isset($keys[$keyIndexes[$key] + 1])) {
    ...
    public static function positionProcess($dataStr)
    {
    ...
    public static function journeyLength($data)
    {
    ...
    public static function isRobot()
    {
        ...
        foreach ($bot_list as $bot) {
            if (strpos($lowerAgent, $bot) !== false) {
                return true;
            }
        }
