...
class FUser
{
    ...
    public $whoIs = 0; //---replace with whoIsUserVO

    //---user access
    public $idkontrol  = false;
    public $pageAccess = false;

    public $pageParamNeededPermission = array(
    ...
    public function inDate($dateStr = null, $returnComplete = false)
    {
        if ($dateStr !== null) {
            if ($date = FSystem::checkDate($dateStr)) {
                ...
                $this->year = $date[0];
                if (!empty($date[1])) {
                    $this->month = sprintf("%02d", $date[1]);
                }

                if (!empty($date[2])) {
                    $this->day = sprintf("%02d", $date[2]);
                }
        ...
        $month = $this->month;
        $day   = $this->day;
        if ($returnComplete && empty($month)) {
            $month = date("m");
        }

        if ($returnComplete && empty($day)) {
            $day = date("d");
        }
    ...
    public function setRemoteAuthToken($v)
    {
    ...
    public static function getToken($tokenizer)
    {
     ...
     * @return void
     */
    public static function login($name, $pass, $pageId = '')
    {
        $name = trim($name);
        $pass = trim($pass);
        if (!empty($name) && !empty($pass)) {
            ...
            if ($pageId != '') {
                FHTTP::redirect(FSystem::getUri('', $pageId, ''));
            }
     ...
     */
    public function check($userVO)
    {
        $ret = false;
        if ($userVO->userId > 0 || !empty($userVO->idlogin)) {
            //---check only if user was logged
            if ($userVO->userId > 0) {
                $q = "select ul.loginId";
            } else {
                $q = "select ul.userId";
    ...
    public function kde()
    {
         ...
         */
        if ($pageAccess === true) {
            //---if page exists continue to test permissions
            //---check if user sent data to login
            if (isset($_POST['lgn']) && $this->idkontrol === false) {
                FUser::login($_POST['fna'], $_POST['fpa'], $this->pageId);
            }
            ...
            if (!FRules::get($userId, $permPage, $permissionNeeded)) {
                ...
                if ($this->itemVO) {
                    $itemAccess = false;
                    if ($this->itemVO->public == 1) {
                        $itemAccess = true;
                    } else if ($this->itemVO->public == 2 && $userId > 0) {
                        $itemAccess = true;
                    } else if ($this->itemVO->public == 0 && $userId == $this->itemVO->userId) {
                        $itemAccess = true;
                    } else if ($this->itemVO->public == 3 && $this->userVO->isFriend($this->itemVO->userId)) {
                        $itemAccess = true;
                    }
    ...
    public function updateTotalItemsNum($updateMy = false)
    {
    ...
    public function setWhoIs($userId)
    {
    ...
    public static function usersList($arr, $ident = '', $label = '')
    {
        if (!empty($arr)) {
            ...
            foreach ($arr as $userVO) {
                ...
                if (!empty($userVO->dateLastVisit)) {
                    $tpl->setVariable('ACTIVITY', $userVO->dateLastVisit);
                }
     ...
     * @return void
     */
    public static function invalidateUsers($userId = 0, $timeOut = USERLIFETIME)
    {
        FDBTool::query("delete from sys_users_logged where DATE_ADD(dateUpdated,INTERVAL " . $timeOut . " MINUTE) < NOW()");
        if ($userId > 0) {
            FDBTool::query("delete from sys_users_logged where userId='" . $userId . "'");
        }
    ...
    public static function register($data)
    {
        ...
        if (strlen($data["jmenoreg"]) < 2) {
            FError::add(FLang::$ERROR_REGISTER_TOSHORTNAME);
        } elseif (strlen($data["jmenoreg"]) > 10) {
            FError::add(FLang::$ERROR_REGISTER_TOLONGNAME);
        } elseif (!FUser::checkUsername($data["jmenoreg"])) {
            FError::add(FLang::$ERROR_REGISTER_NOTALLOWEDNAME);
        } elseif (FUser::isUsernameRegistered($data["jmenoreg"]) || in_array(strtolower($data["jmenoreg"]), $reservedUsernames)) {
            FError::add(FLang::$ERROR_REGISTER_NAMEEXISTS);
        } elseif ($data["jmenoreg"] != $safeJmenoreg) {
            FError::add(FLang::$ERROR_REGISTER_BADUSERNAME . $safeJmenoreg);
        }

        if (strlen($data["pwdreg1"]) < 2) {
            FError::add(FLang::$ERROR_REGISTER_PASSWORDTOSHORT);
        } elseif ($data["jmenoreg"] == $data["pwdreg1"]) {
            FError::add(FLang::$ERROR_REGISTER_PASSWORDNOTSAFE);
        } elseif ($data["pwdreg1"] != $data["pwdreg2"]) {
            FError::add(FLang::$ERROR_REGISTER_PASSWORDDONTMATCH);
        }
        ...
        if (true !== $validator->isValid($data['email'])) {
            FError::add(FLang::$ERROR_INVALID_EMAIL);
        }
        ...
        if (FDBTool::getOne("select count(1) from sys_users where email='" . $db->escape($data['email']) . "'")) {
            FError::add(FLang::$ERROR_USED_EMAIL);
        }
     ...
     * @return String
     */
    public static function getgidname($userId)
    {
     ...
     * @return [0,1]
     */
    public static function isUserIdRegistered($userId)
    {
     ...
     * @return [0,1]
     */
    public static function isUsernameRegistered($name)
    {
