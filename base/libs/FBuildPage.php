...
class FBuildPage
{
    ...
    public static function getTitle()
    {
        ...
        if (!empty($user->pageVO->htmlTitle)) {
            $pageTitle[] = $user->pageVO->htmlTitle;
        } else if (!empty($title)) {
            $pageTitle[] = $title;
        }
    ...
    {
        $user = FUser::getInstance();
        if ($user->pageVO->showHeading === false) {
            return '';
        }
    ...
    public static function getBreadcrumbs()
    {
        ...
        if ($pageTop->pageId) {
            ...
            if (strpos($pageTop->prop('homesite'), 'http:') === false) {
                $homesite = 'http://' . $homesite;
            }
        ...
        if ($pageIdTop != $user->pageVO->pageId) {
            ...
            if (!empty($typeId)) {
                ...
                if ($user->pageVO->categoryId > 0) {
                    $pageCategory = $user->pageVO->categoryId;
                }

            }
            if (empty($pageCategory)) {
                if ($user->categoryVO && $user->pageVO->typeId == 'top') {
                    $pageCategory = $user->categoryVO->categoryId;
                }
            ...
            if ($pageCategory > 0) {
                ...
                if (!empty($categoryArr)) {
                    $pageCrumb = false;
                    if ($typeCrumb == false) {
                        $breadcrumbs[] = array('name' => FLang::$TYPEID[$categoryArr[1]], 'url' => FSystem::getUri('', 'foall', $categoryArr[1]));
                    }
            ...
            //stranka
            if (!empty($user->pageVO->name) && $pageCrumb) {
...
//CATEGORY
        $itemCategory = 0;
        if ($user->categoryVO && $user->pageVO->typeId != 'top') {
            $itemCategory = $user->categoryVO->categoryId;
        }

        if ($user->itemVO) {
            $itemCategory = $user->itemVO->categoryId;
        }
...
//ITEM
        if ($user->itemVO) {
            ...
            if (!empty($user->itemVO->htmlName)) {
                $itemName = $user->itemVO->htmlName;
            }
    ...
    public static function addTab($arrVars)
    {
    ...
    public static function process($data)
    {
        ...
        if ($user->pageAccess == true) {
            if ($user->pageParam == 'sa' || $user->pageParam == 'e') {
    ...
    public static function baseContent($data)
    {
            ...
            switch ($user->pageParam) {
                ...
                case 'h':
                    ...
                    if (empty($user->pageVO->content)) {
                        $user->pageVO->content = FLang::$MESSAGE_FORUM_HOME_EMPTY;
                    }
                ...
                default:
                    $template = $user->pageVO->template;
                    if (empty($template) && $user->pageVO->typeId != 'culture') {
                        $template = 'page_ItemsList';
                    }
    ...
    public static function show($data)
    {
        ...
        if ($user->pageVO) {
            ...
            //searchform
            if ($user->idkontrol && !$user->pageVO->prop('hideSearchbox')) {
            ...
            if ($user->pageVO->pageIdTop != $user->pageVO->pageId) {
                $tpl->setVariable('RSSPAGEID', $user->pageVO->pageId);
            }

            if (!empty($user->pageVO->description)) {
                $tpl->setVariable("DESCRIPTION", str_replace('"', '', $user->pageVO->description));
            }

            if (false !== ($pageHeading = FBuildPage::getHeading())) {
                if (!empty($pageHeading)) {
                    $tpl->setVariable('PAGEHEAD', $pageHeading);
                }
            }
            ...
            if ($user->pageVO->showTopBanner && $user->pageVO->typeId != 'galery' && empty($user->itemVO)) {
                ...
                if (!empty($topbanner)) {
                    $topbannerData = array();
                    $topbanner     = explode("\n", $topbanner);
                    if (strpos($topbanner[0], ';') !== false) {
                        $firstBanner = explode(';', $topbanner[0]);
                    } else {
                        $firstBanner = explode(',', $topbanner[0]);
                    }
                    ...
                    if ($firstBanner[4] > 0) {
                        ...
                        if ($topbannerCache !== false) {
                            $topbannerData = $topbannerCache;
                        }
                    ...
                    if (empty($topbannerData)) {
                        ...
                        if (strpos($firstBanner[0], 'http') === 0) {
                            $topbannerData['i'] = $firstBanner[0];
                        } else {
                    ...
                    if ($topbannerData) {
                        ...
                        if (!empty($topbannerData['u'])) {
                            $tpl->setVariable('TOPBANURL', $topbannerData['u']);
                        }

                        if (!empty($topbannerData['t'])) {
                            $tpl->setVariable('TOPBANTITLE', $topbannerData['t']);
                        }
            ...
            if ($user->pageVO) {
                ...
                if (!empty($lomenuItems)) {
                    foreach ($lomenuItems as $menuItem) {
                        ...
                        if (isset($options['id'])) {
                            $tpl->setVariable('LOID', $options['id']);
                        }

                        if (isset($options['class'])) {
                            $tpl->setVariable('CLASS', $options['class']);
                        }

                        if (isset($options['title'])) {
                            $tpl->setVariable('LOTITLE', $options['title']);
                        }

                        if (isset($options['parentClass'])) {
                            $tpl->setVariable('LISTCLASS', $options['parentClass']);
                        }
