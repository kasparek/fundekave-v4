...
//not valid item
    ...
    public static function showupload($data)
    {
    ...
    public static function edit($data, $itemVO = null)
    {
        if (!$data['__ajaxResponse']) {
            return;
        }
        ...
        if (FRules::getCurrent(2) || ($user->pageVO->pageId == 'event' && FRules::getCurrent(FConf::get('settings', 'perm_add_event')))) {
            if (empty($itemVO) && !empty($data['i'])) {
                $itemVO = new ItemVO($data['i'], true);
            }
            ...
            if (empty($itemVO->typeId) && isset($data['t'])) {
                $itemVO->set('typeId', $data['t']);
            }
            $ret = FItemsForm::show($itemVO);
            FAjax::addResponse(isset($data['target'])?$data['target']:'editForm', '$html', $ret);
            FAjax::addResponse('call', 'jUIInit', '');
        } else {
    ...
    public static function submit($data)
    {
        ...
        if (!empty($data['__ajaxResponse'])) {
            if (FAjax::isRedirecting() === false) {
                ...
                } elseif ($data['t'] == 'galery') {
                	FAjax::addResponse('editForm-'.$data['i'], '$html', '');
                	FAjax::addResponse('image-desc-'.$data['i'], '$html', $data['text']);
                } else {
    ...
    public static function delete($data)
    {
        ...
        if (!$itemVO = FactoryVO::get('ItemVO', $itemId, true)) {
            return;
        }
    ...
    public static function commentsForm($data)
    {
        $user = FUser::getInstance();
    ...
    public static function tempStoreFlush($data)
    {
