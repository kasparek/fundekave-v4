...
function optSetFilter(text) {
    ...
    var z = 0;
    if(hasOptions(obj)) {
        if (optBackup.length==0) {
            for (var j=0; j<obj.options.length; j++) optBackup[j] = new Array(obj.options[j].value,obj.options[j].text);
        }
       ...
       obj.selectedIndex = -1;
        ...
        for (var i=0; i<optBackup.length; i++) {
		  if (re.test(optBackup[i][1])) {
		      obj.options[z] = new Option(optBackup[i][1],optBackup[i][0]);
		      z++;
		  }
		}
