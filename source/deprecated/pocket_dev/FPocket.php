...
class FPocket extends FDBTool {
    ...
    static function getLink($itemId,$page=false) {
        if(FConf::get("pocket","enabled") == 1)
            return '<a href="'.FSystem::getUri('m=user-pocketIn&d='.(($page)?('page='.$itemId):('item='.$itemId))).'" class="fajaxa">'.LABEL_POCKET_PUSH.'</a>';
    }
    function getItems() {
        ...
        if(FConf::get("pocket","enabled") == 1) {
            ...
            if(!empty($arr)) {
                ...
                foreach ($arr as $item) {
                    ...
                    $tpl->setVariable('POCKETTITLE',FText::preProcess($item[5].((isset($index))?(' '.$item[$index]):('')),array('plainText'=>1)));
                    $tpl->setVariable('POCKETNAME',FText::preProcess($nameOnPocket,array('plainText'=>1,'lengthLimit'=>20,'lengthLimitAddOnEnd'=>' ...')));
