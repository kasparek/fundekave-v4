...
function MarkerClusterer(map, opt_markers, opt_options) {
	...
	this.drawnZoomed=false;
	

	// Add the map event listeners
	var that = this;
	google.maps.event.addListener(this.map_, 'zoom_changed', function() {
		var maxZoom = that.maxZoom_ ? that.maxZoom_ : that.map_.mapTypes[that.map_.getMapTypeId()].maxZoom;
		var zoom = that.map_.getZoom();
		
		if (zoom < 0 || (zoom > maxZoom && that.drawnZoomed)) {
			return;
		}

		if (that.prevZoom_ != zoom) {
			that.prevZoom_ = that.map_.getZoom();
			that.drawnZoomed = zoom > maxZoom; 
			that.resetViewport();
		}
	});
 ...
 * @private
 */
MarkerClusterer.prototype.MARKER_CLUSTER_IMAGE_PATH_ =Sett.jsUrl+'markerclusterer/images/m';
