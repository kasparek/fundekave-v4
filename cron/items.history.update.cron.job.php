...
,"select date_format(dateCreated,'%Y-%m-%d') as dateCon,itemId,3,count(itemId) from sys_pages_items_hit where date_format(dateCreated,'%Y%m%d') >= date_format(now(),'%Y%m%d') and userId>0 group by dateCon,itemId"
...
,"select date_format(dateCreated,'%Y-W%U') as dateCon,itemId,3,count(itemId) from sys_pages_items_hit where date_format(dateCreated,'%Y%U') >= date_format(now(),'%Y%U') and userId>0 group by dateCon,itemId"
  ...
  do {
    ...
    if($total > 0) {
      ...
      while($row = mysql_fetch_row($result)) {
        $ins = "insert into sys_pages_items_history values ('".$row[0]."','".$row[1]."','".$row[2]."','".$row[3]."') on duplicate key update valueSum = '".$row[2]."'";
