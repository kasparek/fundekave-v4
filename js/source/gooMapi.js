...
var GooMapi = new function () {
		...
		o.setUnit = function (n, R, id) {
			o.unitR = R;
			o.unit = n;
			if(id) o.editorData(id).updateDistance();
		};
		o.distance = function (lat1, lon1, lat2, lon2) {
			var pr = Math.PI / 180,
				dLat = (lat2 - lat1) * pr,
				dLon = (lon2 - lon1) * pr,
				a = Math.sin(dLat / 2) * Math.sin(dLat / 2) + Math.cos(lat1 * pr) * Math.cos(lat2 * pr) * Math.sin(dLon / 2) * Math.sin(dLon / 2),
		...
		o.posFormat = function (p) {
			...
			if(dir == 'W' || dir == 'E' || dir == 'N' || dir == 'S') {
				var posArr = p.substr(0, p.length - 1).split(' '),
					d = posArr[0] - 0,
					m = posArr.length > 1 ? posArr[1] - 0 : 0,
					s = posArr.length > 2 ? posArr[2] - 0 : 0,
					sign = (dir == 'W' || dir == 'S') ? -1 : 1;
				p = sign * (d + ((m + (s / 60)) / 60));
			}
			return p - 0;
		};
			...
			if(v.length > 0) {
				...
				for(i = 0; i < l.length; i++) {
					...
					if(l[i].length == 2) {
						...
						if(l[i][0] == 0 && l[i][1] == 0) l[i] = false;
					} else {
						l[i] = false;
					}
				}
				for(i = 0; i < l.length; i++) {
					if(l[i] !== false) r.push(l[i]);
				}
			}
			return r;
		};
		...
		o.call = [];
		o.load = function (f) {
			if(o.loaded) return true;
			if(indexOf(o.call, f) == -1) o.call.push(f);
			if(o.loading) return;
			o.loading = true;
		...
		o.init = function () {
			...
			$(".geoInput").each(function () {
				...
				if($(this).is("textarea")) {
					if($('.' + id + 'Thumb').length == 0) {
						$(this).change(o.staticSel).hide().after('<a href="#" class="' + id + 'ThumbA" title="Ukaz na mape"><img class="' + id + 'Thumb" src="" width="' + w + '" height="' + h + '" alt="Google Maps" /></a><a href="#" class="' + id + 'Source" title="Source waypoints"><img src="' + Sett.skinUrl + '/img/source.png" alt="Waypoints source" /></a>');
				...
				} else $(this).unbind('click', o.geoInputClick).click(o.geoInputClick);
			}).change();
			$(".mapLarge").each(function () {
				...
				if(!$(this).hasClass('hidden') && $("map" + id + "holder", this).length == 0) if($(this).hasClass('editor')) {
					var d = o.editorData(id, this),
						rel = $(this).attr('rel');
					if(rel) d.dataEl = document.getElementById(rel);
					d.parent.pop = false;
					o.mapEditor(id);
				} else o.show(id);
			});
		...
		o.geoInputClick = function (e, id) {
			...
			if(id) {
				...
				if(el) {
					if($(el).is('textarea')) d.dataEl = el;
				}
			}
			if(!d.dataEl) $(d.parent.eli.mapSaveB).hide();
			else $(d.parent.eli.mapSaveB).show();
			o.mapEditor('Editor');
			return false;
		};
		o.staticSel = function (e) {
			var p = o.parsePos($(this).val()),
				id = $(this).attr('id'),
				w = $(this).width(),
				h = $(this).height();
		...
		o.editorData = function (id, parent) {
			...
			$(parent ? parent : "body").append('<div style="' + style + '" id="map' + id + 'Overlay">' + '<div style="position:absolute;width:100%;height:20px;background-color:#e78f08;color:#ffffff;font-weight: bold;font-family: Trebuchet MS,Tahoma,Verdana,Arial,sans-serif;font-size:11px;"><div style="padding:0 10px;line-height:20px;">' + (!parent ? '<a id="mapCloseB" href="#" role="button" style="display:block;float:right;color:#ffffff;">' + o.locale.close + '</a>' : '') + '<span class="mapTitle">' + o.locale.title + '</span> <span class="mapUnitsBox"><a class="mapUnitNM" href="#" rel="nm" style="color:#ffffff;" title="' + o.locale.unitTitle + '">NM</a> <a class="mapUnitKm" href="#" rel="km" style="color:#ffffff;" title="' + o.locale.unitTitle + '">Km</a></span></div></div>' + '<div class="map"></div>' + '<div style="bottom:2px;left:5px;position:absolute;"><input class="mapSearchI" value="" style="width:200px;"/><button class="mapSearchB" style="">' + o.locale.search + '</button></div>' + '<div style="bottom:2px;right:5px;position:absolute;"><button class="mapClearB" title="' + o.locale.clearTitle + '">' + o.locale.clear + '</button><button class="mapSaveB" title="' + o.locale.saveTitle + '">' + o.locale.save + '</button></div></div>');
			var topEl = document.getElementById('map' + id + 'Overlay'),
				h = new o.hold($('.map', topEl)[0]);
			h.id = id;
			h.editor = true;
			h.eli.mainEl = topEl;
		...
		o.hold = function (mapEl) {
			...
			h.init = function () {
				if(!h.map) {
					...
					h.map.setZoom(2);
					if(!h.editor) h.cluster = new MarkerClusterer(h.map, [], {
						'maxZoom': 10,
						'zoomOnClick': true
					});
				}
		...
		o.data = function (p) {
			...
			d.addListeners = function () {
				...
				ge.addListener(d.path, 'click', function (e) {
					var min = 999,
						mi, ms = this.data.markers;
					for(var i = 0; i < ms.length - 1; i++) {
						var m1 = ms[i],
							m2 = ms[i + 1],
							diff = Math.abs(google.maps.geometry.spherical.computeDistanceBetween(m1.getPosition(), m2.getPosition()) - (google.maps.geometry.spherical.computeDistanceBetween(e.latLng, m1.getPosition()) + google.maps.geometry.spherical.computeDistanceBetween(e.latLng, m2.getPosition())));
						if(diff < min) {
							min = diff;
							mi = i;
						}
			...
			d.updateMarker = function (latLng, i) {
				...
				if(typeof (i) == 'undefined') i = l > 0 ? l - 1 : 0;
				else if(i > l) i = l;
				if(!d.markers[i]) {
					...
					if(d.parent.cluster) d.parent.cluster.addMarker(m);
				}
				if(d.ico) {
				...
				if(d.infoEl) if(d.infoEl.length > 0) m.htmlInfo = $(d.infoEl).html();
				$(d.infoEl).hide();
			};
			d.resetWP = function () {
				if(d.path) d.path.setPath([]);
				while(d.markers.length > 0) d.markers.pop().setMap(null);
			};
			d.addWP = function (latLng, wm) {
				...
				if(wm) d.updateMarker(latLng, d.markers.length);
			};
			d.updateDistance = function () {
				...
				if(d.parent.editor) {
					$(d.parent.eli.mapTitle).html(d.distance > 0 ? o.locale.distance + d.distance + o.unit : o.locale.title);
					if(d.distance > 0) $(d.parent.eli.mapUnitsBox).show();
					else $(d.parent.eli.mapUnitsBox).hide();
				}
			};
			d.get = function () {
				if(!d.dataEl) return [];
			...
			while(o.showQueue.length > 0) {
				...
				for(var i = 0; i < h.li.length; i++) {
					...
					if(ll > 0) {
						...
						if(o.poLi[po]) {
							var a = Math.ceil(o.poLi[po] / 4),
								b = 4 - ((a * 4) - o.poLi[po]);
							if(b == 1) l[ll - 1][0] += a * 0.0001;
							else if(b == 2) l[ll - 1][1] += a * 0.0002;
							else if(b == 3) l[ll - 1][0] -= a * 0.0001;
							else l[ll - 1][1] -= a * 0.0002;
							o.poLi[po]++;
						} else o.poLi[po] = 1;
						for(var j = 0; j < ll; j++) {
					...
					if(!d.parent.editor && ll > 0) {
						...
						if(d.infoEl.length > 0) {
							...
							google.maps.event.addListener(m, 'click', function (e) {
								if(!o.info) o.info = new google.maps.InfoWindow({
									maxWidth: 300
								});
		...
		o.editorClick = function (e) {
			var d = o.editorData(e.id ? e.id : this.data.id),
				r = d.path ? d.path.getPath() : null,
				add = true,
				l = 0;
			if(r) l = r.getLength();
			if(l > 0) if(r.getAt(l - 1).lat() == e.latLng.lat() && r.getAt(l - 1).lng() == e.latLng.lng()) add = false;
			if(add) d.addWP(e.latLng, true);
			d.updateDistance();
		};
		o.close = function (e) {
		...
		o.save = function (e) {
			...
			if(d.dataEl) {
				var oldVal = $(d.dataEl).val(),
					newVal = o.toString(this.data.id);
				$(d.dataEl).val(newVal);
				if(oldVal != newVal) $(d.dataEl).change();
			}
			o.close();
		};
		...
		o.clear = function (e) {
			...
			d.resetWP();
			d.updateDistance();
			$(d.parent.eli.mapTitle).html(o.locale.title);
		};
		o.wkey = function (e) {
			if(e.keyCode == 27) o.close();
		}
		o.searchKey = function (e) {
			if(e.keyCode == 13) o.search(e);
		};
		...
		o.search = function (e) {
			var d = o.editorData(e.target.data.id),
				valI = d.parent.eli.mapSearchI.value,
				pos = o.parsePos(valI);
			if(pos.length > 0) o.searchResultHandler({
			...
			else if(d.parent.geocoder) d.parent.geocoder.geocode({
				address: valI
			}, function (results, status) {
				if(status == google.maps.GeocoderStatus.OK) o.searchResultHandler(results[0].geometry, e.target.data.id);
			});
		};
	};