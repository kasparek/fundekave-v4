var Fajax = new function () {
		...
		o.init = function () {
			if($(".fajaxform").length > 0) {
				Lazy.load(Sett.ll.form);
				listen('button', 'click', Fajax.form);
			}
			if($(".fajaxpager").length > 0) listen('fajaxpager', 'click', o.pager);
		...
		o.a = function (e) {
			...
			if(t.hasClass('progress')) {
				var bar = $(".showProgress"),
					h = bar.height();
				bar.addClass('lbLoading').css('height', (h > 0 ? h : $(window).height()) + 'px').children().hide();
			}
			if(t.hasClass('hash')) {
				Hash.set(action);
				return false;
		...
		o.action = function (action) {
			...
			if(id) {
				if(!res) o.add('result', id);
				if(!prop) o.add('resultProperty', '$html');
			}
			o.send(m, k);
			return false;
		};
		o.form = function (e) {
			var t = e.currentTarget,
				jt = $(t);
			if(jt.hasClass('noFajax')) return;
			e.preventDefault();
			if(o.formStop == true) {
				o.formStop = false;
				return false;
			}
			if(jt.hasClass('confirm')) if(!confirm(jt.attr("title"))) return false;
			$('#errormsgJS').hide(0).html('');
			$('#okmsgJS').hide(0).html('');
			o.formSent = t.form;
			var arr = $(o.formSent).formToArray(false),
				action, res = false,
				prop = false;
			while(arr.length > 0) {
				var v = arr.shift();
				if(v.name == 'm') action = v.value;
				else o.add(v.name, v.value);
				if(v.name == 'result') res = true;
				if(v.name == 'resultProperty') prop = true;
			}
			if(!res) o.add('result', $(o.formSent).attr("id"));
			if(!prop) o.add('resultProperty', '$html');
			o.add('action', t.name);
					...
					$(a.responseXML).find("Item").each(function () {
						...
						switch(target) {
							...
							case 'call':
								var par = text.split(','),
									p = [];
								for(var i = 0; i < par.length; i++) p.push("par[" + i + "]");
							...
							default:
								switch(prop) {
									...
									default:
										if(prop[0] == '$') {
											c = '$("#' + target + '").' + prop.replace('$', '') + '(text);';
										} else {
											c = '$("#' + target + '").attr("' + prop + '",text);';
										}
								};
						};
