...
function fuupInit(){if($(".fuup").length>0){if(!Lazy.load(Sett.ll.swf,fuupInit))return;$(".fuup").each(function(i){
	var id=$(this).attr('id'),fuupConf=$.base64.encode(JSON.stringify(FuupConfig[id]));
...
function fuupUploadComplete(k,v){if(k=='error')alert(Sett.locale.galery[v]);if(k!='complete')return; var i=$('#i').attr('value');if(i>0)Fajax.add('i',i);Fajax.add('call','jUIInit');Fajax.send('item-image',Sett.page);}
...
var GaleryEdit=new function(){var o=this;o.uploadStatus='';o.numToUpload=0;o.numTotal=0;o.numLoaded=0;o.newLi=[];o.updLi=[];o.run=false;
o.init=function(){$("#uploadButt").unbind('click',Fajax.form).addClass('noFajax').click(function(){$("#fuga")[0].fuupGateIn("upload");return false;});$("#cancelButt").unbind('click',Fajax.form).addClass('noFajax').click(function(){$("#fuga")[0].fuupGateIn("cancel");return false;});$("#removeAllButt").unbind('click',Fajax.form).addClass('noFajax').click(function(){$("#fuga")[0].fuupGateIn("removeAll");return false;});o.numLoaded=0;o.numTotal=parseInt($("#fotoTotal").text());if(o.numTotal>0 && $('#fotoList').length>0)o.load(0,10);};
o.updateControls=function(){if(o.uploadStatus=='statusBusy'){$("#uploadButt").removeClass('ui-state-hover').button('disable');$("#cancelButt").button('enable');$("#removeAllButt").button('disable').removeClass('ui-state-hover');}else{$("#cancelButt").button('disable').removeClass('ui-state-hover');if(o.numToUpload>0){$("#uploadButt").button('enable');$("#removeAllButt").button('enable');}else{$("#uploadButt").removeClass('ui-state-hover').button('disable');$("#removeAllButt").button('disable').removeClass('ui-state-hover');}}};
o.check=function(k,v){console.log("FUUP::"+k+" '"+v+"'");switch(k){case 'imageUploaded':Fajax.send('page-fuup',Sett.page);break;case 'imageNum':o.numToUpload=parseInt(v);o.updateControls();break;case 'status':o.uploadStatus=v;o.updateControls();if(v=="statusReady" && $("#uploadControlsHeading").length==0){$('<h2 id="uploadControlsHeading">'+Sett.locale.galery.selectFiles+'</h2>').hide().appendTo("#uploadControls").slideDown("slow");}break;case 'error':alert(Sett.locale.galery[v]);break;}};
...
$(".geoInput").each(function(){var id=$(this).attr('id'),w=$(this).width(),h=$(this).height();if(!id){id=o.uid();$(this).attr('id',id);}if($(this).is("textarea")){if($('.'+id+'Thumb').length==0){$(this).change(o.staticSel).hide().after('<a href="#" class="'+id+'ThumbA" title="Show route"><img class="'+id+'Thumb" src="" width="'+w+'" height="'+h+'" alt="Google Maps" /></a><a href="#" class="'+id+'Source" title="Source waypoints"><img src="'+Sett.skinUrl+'/img/source.png" alt="Waypoints source" /></a>');$('.'+id+'Source').click(function(){clearTimeout(o.sourceTOut);$('#'+$(this).attr('class').replace('Source','')).toggle().change();return false;}).hide();$('.'+id+'Thumb').click(function(){o.geoInputClick(null,$(this).attr('class').replace('Thumb',''));});$('.'+id+'ThumbA').hover(o.sourceOver,o.sourceOut).click(function(){return false;});}} else $(this).unbind('click',o.geoInputClick).click(o.geoInputClick);}).change();
