...
class JSCompiler {
    ...
    if (!$this->_cache_output) {
      ...
      if ($this->_isRecompileNeeded($cache_file)) {
        ...
        if ($result !== false)
		  ...
		  if((bool) $this->_outputFilename) file_put_contents($this->_outputFilename, $result);
        echo $result;
      } else {
  ...
  function _localCompile() {
    $js_cmd = 'java -jar '.getcwd().'/_compiler/compiler.jar';
    ...
    $soy_cmd = 'java -jar ' . getcwd() . '/_compiler/SoyToJsSrcCompiler.jar';
  ...
  function _getCacheFileName() {
	if(!empty($this->_srcsDir)) {
		return $this->_cache_dir . str_replace('/','_',str_replace($this->_cache_dir,'',$this->_srcsDir)).'.min.js';
	} else if(count($this->srcs)==1) {
		return $this->_cache_dir . str_replace('.js','.min.js',$this->srcs[0]);
	} else {
