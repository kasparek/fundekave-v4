jQuery.base64=function(a){function b(a,c){var b="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/".indexOf(a.charAt(c));if(-1===b)throw"Cannot decode base64";return b}function c(a,c){var b=a.charCodeAt(c);if(255<b)throw"INVALID_CHARACTER_ERR: DOM Exception 5";return b}return{decode:function(a){var c=0,e,g,h=a.length,k=[];a=String(a);if(0===h)return a;if(0!==h%4)throw"Cannot decode base64";"="===a.charAt(h-1)&&(c=1,"="===a.charAt(h-2)&&(c=2),h-=4);for(e=0;e<h;e+=4)g=b(a,e)<<18|b(a,
...
a.add(d[0],d[1]),"result"==d[0]&&(g=!0),"resultProperty"==d[0]&&(h=!0);e&&(g||a.add("result",e),h||a.add("resultProperty","$html"));a.send(c,f);return!1};a.form=function(b,c){var d=b?b.currentTarget:null,f=d?$(d):null;if(!(!b&&!c||b&&!$(d.form).hasClass("fajaxform")||f&&f.hasClass("noFajax"))){b&&b.preventDefault();if(!0===a.formStop)return a.formStop=!1;if(f&&f.hasClass("confirm")&&!confirm(f.attr("title")))return!1;window.tinymce&&tinymce.triggerSave();a.formSent=c?c:d.form;for(var f=$(a.formSent).serializeArray(),
...
500),topBannerHeight=0);return!1});isMobile||setTimeout(function(){0<$("#galeryFeed").length&&Fajax.send("page-thumbs",_fdk.cfg.page)},1E3);0<$("#head-banner").length&&($(window).resize(topBannerPosition).resize(),$(".top-image img").on("load",topBannerPosition));$('a[data-toggle="tab"]').on("shown.bs.tab",function(a){var b=$(a.target).attr("href"),f=$(a.target).data("target");f&&$(f).is(":empty")&&$.ajax({type:"GET",url:b.replace("show","show-x"),complete:function(a){a.responseText&&$(f).html(a.responseText)}})});
...
"statechange",function(){var a=History.getState();History.log("statechange:",a.data,a.title,a.url);0<parseInt(a.data.i)&&(Fajax.xhrList["item-show"]&&(History.log("cancel ajax:","item-show"),Fajax.cancel("item-show")),isMobile||$(".gallery").flickity("select",$("div[data-itemid='"+a.data.i+"']").index()),Fajax.action(a.data.action+"/i="+a.data.i+"/0/"+a.data.eid))});listen("history","click",function(a){var b=gup("i",a.currentTarget.href);History.pushState({action:"item-show",eid:$(a.target).attr("id"),
...
$("#recipientList").change(function(a){var b=[];$("#recipientList option:selected").each(function(){b.push($(this).text())});b&&$("#recipient").val(b.join(",")).change();avatarfrominput()});GaleryEdit.init(_fdk.cfg,_fdk.lng.galery);0<parseInt(_fdk.cfg.msgTi)&&Msg.startPoll();var b=$("#accessSel");0<b.length&&b.change(function(){0==$(this).val()?$("#rule1").show():$("#rule1").hide()}).change()}}}
...
a.currentIndex=0;a.numCell=0;a.buttonTriggerScroll=!1;a.init=function(){isMobile?($(window).on("scroll",function(){clearTimeout($.data(this,"scrollTimer"));$.data(this,"scrollTimer",setTimeout(function(){console.log("scroll stopped");if(a.buttonTriggerScroll)a.buttonTriggerScroll=!1;else{var b=0;$(".gallery-cell").each(function(){$i=$(this);$w=$(window);var d=$w.scrollTop();$w.height();var f=$i.offset().top;$i.height();if(f>=d)return a.currentIndex=b,!1;b++})}},250))}),$("#galleryNext").on("click",
function(){0===a.numCell&&(a.numCell=$("div.gallery-cell").length);a.currentIndex++;a.currentIndex>=a.numCell&&(a.currentIndex=0);var b=$($("div.gallery-cell")[a.currentIndex]);if(Fullscreen.isFullscreen){$(".gallery").prepend(b);a.lazyLoad($("img",b));var d=$($("div.gallery-cell")[a.currentIndex+1]);d&&a.lazyLoad($("img",d))}else a.buttonTriggerScroll=!0,$("html, body").animate({scrollTop:b.offset().top-($(window).height()-b.height())/2},600);b=b.data("itemid");History.pushState({action:"item-show",
eid:b,i:b},"Loading ...","?i="+b)}),$("#galleryPrev").on("click",function(){0===a.numCell&&(a.numCell=$("div.gallery-cell").length);a.currentIndex--;0>a.currentIndex&&(a.currentIndex=0);var b=$($("div.gallery-cell")[a.currentIndex]);Fullscreen.isFullscreen?($(".gallery").prepend(b),a.lazyLoad($("img",b))):$("html, body").animate({scrollTop:b.offset().top-($(window).height()-b.height())/2},600);b=b.data("itemid");History.pushState({action:"item-show",eid:b,i:b},"Loading ...","?i="+b)})):($(".gallery").flickity({wrapAround:!0,
pageDots:!1}),$(".bottlef").remove(),$(".bottrig").remove());$(window).resize(a.on).resize();var b=$("img.gallery-cell-image:first")[0];if(b.complete)isMobile?a.unveilInit():setTimeout(a.doQueue,500);else $(b).on("load",function(){isMobile?a.unveilInit():setTimeout(a.doQueue,500)});$(".gallery img").load(function(){var b=$(window),d=b.width(),b=b.height();imgResizeToFit($(this),Fullscreen.isFullscreen?d:$("#fullscreenBox").width(),b-0,Fullscreen.isFullscreen);ImgNext.hud();a.isQueue&&0<a.queue.length&&
a.nextQueue()}).error(function(){var a=(new Date).getTime(),a=$(this).attr("src")+"?"+a;$(this).attr("src",a);console.log("Error Loading Image "+a)});$(".js-flickity").on("cellSelect",function(){var b=$(".gallery").data("flickity");if(b){var d=b.selectedElement,b=!0;a.flktySelected||(b=!1);a.flktySelected==d&&(b=!1);a.flktySelected=d;b&&(b=$("img",d),$(b).data("lazy")&&($(b).attr("src",$(b).data("lazy")),$(b).data("lazy",null)),a.flktyTimeout&&clearTimeout(a.flktyTimeout),a.flktyTimeout=setTimeout(function(){var a=
$(d).data("itemid");History.pushState({action:"item-show",eid:a,i:a},"Loading ...","?i="+a)},1E3))}})};a.force=function(){a.cw=a.ch=0;a.on()};a.on=function(){var b=$(window),c=b.width(),d=b.height();if(c!=a.cw||d!=a.ch)a.cw=c,a.ch=d,ImgNext.hud();$(".gallery img").each(function(a){imgResizeToFit($(this),Fullscreen.isFullscreen?c:$("#fullscreenBox").width(),d-0,Fullscreen.isFullscreen,Fullscreen.isFullscreen)})}},ImgNext=new function(){var a=this;a.r=!1;a.i=null;a.il=[];a.p=null;a.next=[];a.nav=[];
...
var c=a.width(),a=a.height(),a=$(window).height(),d=0;$(".gallery-cell img").each(function(){var a=$(this).height();a>d&&(d=a)});isMobile||$(".gallery").width(c).height(d);History.log("ImgNext.hud",c+"x"+a)};a.xhrHand=function(b,c,d,f,e){a.init();a.nav[b]=[c,d];f&&f!=a.i.attr("src")&&a.i.attr("src",f).data("i",b);e&&($("body").append('<img id="imgNextLoader'+c+'" class="noscreen" />'),a.il[c]=$("#imgNextLoader"+c).load(a.preloaded),a.il[c].attr("src",e).data("i",c))};a.preloaded=function(b){b=$(b.target);
