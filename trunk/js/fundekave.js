...
o.sourceTOut=null;
o.sourceT=1000;
o.sourceTF=function(el){$('.'+$(el).attr('rel')+'Source').show();};
o.sourceOver=function(e){o.sourceTOut=setTimeout(o.sourceTF,o.sourceT,this);};
o.sourceOut=function(){clearTimeout(o.sourceTOut);};
o.init=function(){o.locale=Sett.locale.goomapi;$(".geoInput").hide().unbind('change',o.staticSel).change(o.staticSel).each(function(){var id=$(this).attr('id'),w=$(this).width(),h=$(this).height();$(this).hide().after('<img class="'+id+'Thumb" src="" width="'+w+'" height="'+h+'" alt="Google Maps" /><a href="#" class="'+id+'Source" title="Source waypoints"><img src="'+Sett.skinUrl+'/img/source.png" alt="waypoints source" /></a>');$('.'+id+'Source').click(function(){clearTimeout(o.sourceTOut);$('#'+$(this).attr('class').replace('Source','')).toggle().change();return false;}).hide();$('.'+id+'Thumb').click(function(){$('.geoselector[rel='+id+']').click()});}).change();        listen('geoselector','mouseenter',o.sourceOver);listen('geoselector','mouseleave',o.sourceOut);listen('geoselector','click',o.geoSelectorClick);listen('mapThumbLink','click',o.thumbClick);$(".mapLarge").each(function(){var id=$(this).attr('id');if(!$(this).hasClass('hidden') && $("map"+id+"holder",this).length==0)if($(this).hasClass('editor')){var d=o.editorData(id,this),rel=$(this).attr('rel');if(rel)d.dataEl=document.getElementById(rel);d.parent.pop=false;o.mapEditor(id);} else o.show(id);});};
...
o.staticSel=function(e){var t=$.trim($(this).val()),p=t.length>0?t.split("\n"):[],id=$(this).attr('id'),w=$(this).width(),h=$(this).height();$('.'+id+'Thumb').attr('src',p.length==0?'':'http://maps.google.com/maps/api/staticmap?size='+w+'x'+h+'&markers='+p[p.length-1]+'&maptype=terrain&sensor=false'+(p.length>1?'&path='+p.join('|'):''));if(!$(this).is(':visible')){if(p.length>0)$('.'+id+'Thumb').show();else $('.'+id+'Thumb').hide();} else $('.'+id+'Thumb').hide();};
...
h.init=function(){if(!h.map){h.geocoder=new google.maps.Geocoder();h.map=new google.maps.Map(h.mapEl,{mapTypeId:h.mapType});h.map.data=h;h.map.setCenter(new google.maps.LatLng(30,20));h.map.setZoom(2);if(!h.editor)h.cluster=new MarkerClusterer(h.map,[],{'maxZoom':10,'zoomOnClick':true});}};};
o.data=function(p){var d=this;d.pathColor="#0000ff";d.pathAlpha="0.5";d.parent=p;d.dataEl=null;d.title='';d.infoEl=null;d.ico=null;d.markers=[];d.path=null;d.distance=0;d.addListeners=function(){if(!d.parent.editor)return;var ge=google.maps.event;ge.clearListeners(d.parent.map,'click');ge.addListener(d.parent.map,'click',o.editorClick);if(d.markers.length==0)return;ge.clearListeners(d.path);ge.addListener(d.path,'click',function(e){var min=999,mi,ms=this.data.markers;for(var i=0;i<ms.length-1;i++){var m1=ms[i],m2=ms[i+1],diff=Math.abs(google.maps.geometry.spherical.computeDistanceBetween(m1.getPosition(),m2.getPosition())-(google.maps.geometry.spherical.computeDistanceBetween(e.latLng, m1.getPosition())+google.maps.geometry.spherical.computeDistanceBetween(e.latLng, m2.getPosition())));if(diff<min){min=diff;mi=i;}}ms.splice(mi+1,0,null);this.getPath().insertAt(mi+1,e.latLng);this.data.updateMarker(e.latLng,mi+1);this.data.updateDistance();});for(var i=0;i<d.markers.length;i++){var m=d.markers[i];ge.clearListeners(m);ge.addListener(m,'drag',function(){this.data.path.getPath().setAt(indexOf(this.data.markers,this),this.getPosition());});ge.addListener(m,'dragend',function(){this.data.path.getPath().setAt(indexOf(this.data.markers,this),this.getPosition());this.data.updateDistance();});ge.addListener(m,'dblclick',function(){var i=indexOf(this.data.markers,this);this.data.path.getPath().removeAt(i);this.data.markers.splice(i,1);this.setMap(null);this.data.updateDistance();this.data=null;});}};d.updateMarker=function(latLng,i){var l=d.markers.length;if(typeof(i)=='undefined')i=l>0?l-1:0; else if(i>l)i=l;if(!d.markers[i]){d.markers[i]=new google.maps.Marker(d.parent.editor?{title:d.title,draggable:true,raiseOnDrag:false}:{title:d.title});var m=d.markers[i];m.data=d;if(d.parent.editor){d.addListeners();m.setTitle(o.locale.markerTitle);}if(d.parent.cluster)d.parent.cluster.addMarker(m);}if(d.ico){m.setIcon(d.ico);m.setZIndex(1);}m.setPosition(latLng);m.setMap(d.parent.map);if(d.infoEl)if(d.infoEl.length>0)m.htmlInfo=$(d.infoEl).html().hide();};d.resetWP=function(){if(d.path)d.path.setPath([]);while(d.markers.length>0)d.markers.pop().setMap(null);};d.addWP=function(latLng,wm){if(!d.path){d.path=new google.maps.Polyline({map:d.parent.map,path:[],strokeColor:d.pathColor,strokeOpacity:d.pathAlpha,strokeWeight:4,geodesic:true});d.path.data=d;}d.path.setMap(d.parent.map);d.path.getPath().push(latLng);if(wm)d.updateMarker(latLng,d.markers.length);};d.updateDistance=function(){d.distance=0;if(!d.path)return;var l=d.path.getPath();if(l.length>1){for(i=1;i<l.length;i++){d.distance+=o.distance(l.getAt(i-1).lat(),l.getAt(i-1).lng(),l.getAt(i).lat(),l.getAt(i).lng());}}d.distance=Math.round(d.distance*10)/10;if(d.parent.editor){$(d.parent.eli.mapTitle).html(d.distance>0?o.locale.distance+d.distance+o.unit:o.locale.title);if(d.distance>0)$(d.parent.eli.mapUnitsBox).show(); else $(d.parent.eli.mapUnitsBox).hide();}};d.get=function(){if(!d.dataEl)return [];return o.parsePos($(d.dataEl).val());};};
...
o.resize=function(e){var w=$(window).width(),h=$(window).height(),mw=w*0.9,mh=h*0.9,mx=(w-mw)/2,my=(h-mh)/2;$("#mapEditorOverlay").css('width',mw+'px').css('height',mh+'px').css('left',mx+'px').css('top',my+'px');};
