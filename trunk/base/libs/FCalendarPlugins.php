...
class FCalendarPlugins {
  static function diaryRecurrenceItems($year,$month,$userId,$pageId = '') {
	...
	'year' as rep
    from sys_pages_items where (userId='".$userId."' or public=1) and ((month(dateStart)=".($month*1)." and textLong = 'year') or (textLong = 'month'))";
	return $db->getAll($q);
  }
  static function events($year,$month,$userId,$pageId = '') {
    $db = FDBConn::getInstance();
    return $db->getAll("select itemId as id,
    addon as title,
    date_format(dateStart ,'%Y-%m-%d') as date, 
    date_format(dateStart ,'%d.%m.%Y') as dateLocal 
    from sys_pages_items where typeId='event' and dateStart like '".$year."-".$month."%'");
  }
  static function blogItems($year,$month,$userId,$pageId = '') {
    ...
    date_format(dateStart ,'%Y-%m-%d') as date, 
    date_format(dateStart ,'%d.%m.%Y') as dateLocal 
    from sys_pages_items where typeId='blog' and ".($pageId!=''?"pageId='".$pageId."' and ":"")."dateStart <= '".$year."-".$month."%' and (itemIdTop is null or itemIdTop=0)
  ...
  static function forums($year,$month,$userId,$pageId = '') {
    ...
    $fPages->setSelect("p.pageId as id,
    p.name as title,
    date_format(p.dateCreated ,'{#date_iso#}') as date, 
    date_format(p.dateCreated ,'{#date_local#}') as dateLocal 
    ");
