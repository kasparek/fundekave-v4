...
if(strpos($_SERVER['REQUEST_URI'],"/pic/")===0 || strpos($_SERVER['REQUEST_URI'],"/pic.php")!==false) {
	include('pic.php');
	exit;
}
...
if(strpos($_SERVER['REQUEST_URI'],"/rss")!==false || strpos($_SERVER['REQUEST_URI'],"/frss.php")===0) {
	FRSS::process($_GET);
	FRSS::build($_GET);
	$processMain = false;
}
